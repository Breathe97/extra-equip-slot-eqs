...
local items_anim_symbols = {} -- 记录物品的贴图数据 item.prefab → {build, symbol}
...
local function InitPrefab()
    ...
    AddComponentPostInit("inventory", function(comp)
        ...
        function comp.Equip(self, item, ...)
            ...
            -- 造一个替身对象，只拦截 OverrideSymbol 和 OverrideItemSkinSymbol
            -- 其他方法通过 __index 自动转发到 AnimState_orig
            local AnimState_proxy = {}
            -- 无皮肤
            AnimState_proxy.OverrideSymbol = function(_, slot, sym_build, symbol)
                if slot == "swap_body" or slot == "swap_body_tall" or slot == "swap_hat" then
                    items_anim_symbols[prefab] = { sym_build, symbol }
                end
                AnimState_orig.OverrideSymbol(AnimState_orig, slot, sym_build, symbol)
            end
            -- 有皮肤
            AnimState_proxy.OverrideItemSkinSymbol = function(_, slot, skin_build, symbol, item_guid, sym_build)
                if slot == "swap_body" or slot == "swap_body_tall" or slot == "swap_hat" then
                    items_anim_symbols[prefab] = { sym_build, symbol }
                end
                AnimState_orig.OverrideItemSkinSymbol(AnimState_orig, slot, skin_build, symbol, item_guid, sym_build)
            end
            setmetatable(AnimState_proxy, { __index = AnimState_orig })

            owner.AnimState = AnimState_proxy
            local result = Equip_orig(self, item, ...)
            owner.AnimState = AnimState_orig
            return result
        end
    end)
end
...
local function RepairExtra()
    ...
    if GLOBAL.EQUIPSLOTS.BACK or GLOBAL.EQUIPSLOTS.HAT then
        ...
        AddPlayerPostInit(function(player)
            ...
            -- 从 items_anim_symbols 查 build 和 symbol，有皮肤走皮肤覆盖
            local function SetSymbol(render_slot, item)
                if item == nil then return end
                local data = items_anim_symbols[item.prefab]
                if data == nil then return end

                local sym_build, symbol = data[1], data[2] -- 读取之前村的 build、symbol
                local skin_build = item.GetSkinBuild(item) -- 有没有皮肤
                if skin_build and skin_build ~= '' then
                    player.AnimState.OverrideItemSkinSymbol(player.AnimState, render_slot, skin_build, symbol, item.GUID, sym_build)
                else
                    player.AnimState.OverrideSymbol(player.AnimState, render_slot, sym_build, symbol)
                end
            end
            ...
            -- 设置头部贴图
            local function SetHeadSymbol(inv)
                local item = inv.GetEquippedItem(inv, GLOBAL.EQUIPSLOTS.HEAD) or inv.GetEquippedItem(inv, GLOBAL.EQUIPSLOTS.HAT)
                if item == nil then return end
            ...
            -- 设置身体栏贴图
            local function SetBodySymbol(inv)
                local item = inv.GetEquippedItem(inv, GLOBAL.EQUIPSLOTS.BODY) or inv.GetEquippedItem(inv, GLOBAL.EQUIPSLOTS.BELLY) or inv.GetEquippedItem(inv, GLOBAL.EQUIPSLOTS.NECK)
                if item == nil then return end
                SetSymbol('swap_body', item)
            end

            -- 设置背包栏贴图
            local function SetBackSymbol(inv)
                local item = inv.GetEquippedItem(inv, GLOBAL.EQUIPSLOTS.BACK)
                if item == nil then return end
